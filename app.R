...
df_barchart   <- read.csv("df_barchart.csv",   stringsAsFactors = FALSE)
...
ui <- dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(id = "sidebarmenu",
        ...
        conditionalPanel("input.sidebarmenu === 'barchart'",
                         sliderInput("slider.barchart", "Top:", min=1, max=length(unique(df_barchart$Reporter_description)), value=10),
                         selectInput("ind.bar.chart", "Indicator:", unique(df_map$Indicator_description))
                        ),
...
server <- function(input, output) {
  ...
  sub_df_barchart <- reactive({
    df_2 <- df_barchart[df_barchart$Year == input$year &
                        df_barchart$Indicator_description == input$ind.bar.chart, ]
    na.omit(df_2[1:min(input$slider.barchart, nrow(df_2)), ])
  })
